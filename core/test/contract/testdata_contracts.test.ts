...
import { listInputTestDataPaths, testInputDirectory } from '@graphics-workbench/core/testing';

const expectedSourceTestDataCounts: Partial<Record<SourceFormat, number>> = {
  avif: 2,
  drawio: 2,
  'drawio-png': 1,
  'drawio-svg': 1,
...
describe('変換用テスト入力の、形式ごとの件数が期待値テーブルと一致する', () => {
  it('validテスト入力ディレクトリを形式ごとに走査し、sourceFormatForPathで判定した形式別の件数が期待値テーブルと一致する', async () => {
    const sourcePaths = (
      await Promise.all(
        [...new Set(Object.keys(expectedSourceTestDataCounts).map(inputDirectoryForFormat))].map((format) =>
          listInputTestDataPaths(path.join(testInputDirectory, 'valid', format)),
        ),
      )
    ).flat();
    ...
    assert.deepStrictEqual(actualCounts, expectedSourceTestDataCounts);
  });
});

function inputDirectoryForFormat(format: string): string {
  return format === 'drawio-png' || format === 'drawio-svg' ? 'drawio' : format;
}
...
function countDefinedFormats(formats: (SourceFormat | undefined)[]): Partial<Record<SourceFormat, number>> {
  ...
  for (const format of formats) {
    assert.ok(format, 'Every source テストデータ must use a supported input format.');
