...
describe('分割出力のpageテンプレート', () => {
  it("総数が9なら'1'、12なら'01'、125なら'012'と、総数の桁数に合わせて1始まりのページ番号を0埋めする", () => {
  ...
  it('出力ページ数が2以上の場合はテンプレートに${page}変数を要求して例外を投げ、${page}を含めば許可し、ページ数が1なら${page}がなくても許可する', () => {
  ...
  it('combine出力テンプレートに${random}変数を要求し、なければ例外を投げる', () => {
