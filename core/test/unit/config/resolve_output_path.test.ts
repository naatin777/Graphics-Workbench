...
describe('出力パスのテンプレート解決', () => {
  it("元PDFパスから${fileDirname}・${fileBasenameNoExtension}・${fileExtname}を展開し、workspace/figures配下に'sample-crop.pdf'の出力パスを生成する", () => {
  ...
  it("相対テンプレート'generated/${relativeFileDirname}/${fileBasename}'をworkspaceパス基準の絶対パス'workspace/generated/figures/sample.pdf'へ解決する", () => {
    const workspacePath = path.resolve(path.sep, 'workspace');
    const sourcePath = path.join(workspacePath, 'figures', 'sample.pdf');
  ...
  it("ソースファイル名に含まれる'${fileExtname}'リテラルを基底名として展開した結果に現れるテンプレート構文を再展開せず、そのままの文字列として出力パスに残す", () => {
    const workspacePath = path.resolve(path.sep, 'workspace');
  ...
  it("未対応のテンプレート変数${unknown}を含む場合は、パス解決を開始せず'Unsupported output path variable'の例外を投げる", () => {
    const workspacePath = path.resolve(path.sep, 'workspace');
  ...
  it('${random}を16進数8桁のランダム文字列へ展開し、workspaceFolder基準の結合PDFパスを生成する', () => {
    const workspacePath = path.resolve(path.sep, 'workspace');
  ...
  it('context.randomを指定すると${random}をその値へ展開する', () => {
    const workspacePath = path.resolve(path.sep, 'workspace');
  ...
  it("Windowsでpath componentに予約文字(< > : \" | ? *)のいずれかを含む場合は、'reserved character'エラーで拒否する", () => {
  ...
  it("WindowsでパスにNUL文字または制御文字(U+0001〜U+001F)を含む場合は、'control character|NUL'エラーで拒否する", () => {
    for (const character of ['\u0000', '\u0001', '\u001f']) {
      assert.throws(
        () =>
          resolveOutputPathWithPlatform(`output/result${character}.pdf`, windowsContext(), {
  ...
  it("WindowsでCON・NUL・COM1・LPT9等の予約デバイス名を、拡張子付き・大文字小文字違い・¹³などの数字置換を含む形でも'reserved name'エラーで拒否する", () => {
  ...
  it("Windowsでpath componentの先頭半角空白・末尾半角空白・末尾ピリオドを含む場合は、それぞれ'leading ASCII space'等の理由で拒否する", () => {
    const cases = [
  ...
  it("Windowsで'${workspaceFolder}\\output\\result.pdf'を解決し、drive letterとバックスラッシュ区切りの絶対パス'C:\\workspace\\output\\result.pdf'を返す", () => {
  ...
  it('Windowsで多言語・絵文字・全角文字・全角空白を含むファイル名を一切変更せず、そのままの文字列で出力パスを返す', () => {
  ...
  it("POSIXではWindows専用の禁止文字と予約名'CON?:*'を含むパスをそのまま許可し、スラッシュ区切りの絶対パスで返す", () => {
  ...
  it("POSIXでもパスにNUL文字を含む場合は、'Invalid output path for POSIX:.*NUL'エラーで拒否する", () => {
    assert.throws(
      () =>
  ...
  it('許容拡張子として.pngのみ指定された場合、.pdfの出力は拡張子エラーで拒否し、大文字の.PNGは許可する', () => {
    assert.throws(
  ...
  it("Draw.io compoundソース名'diagram.drawio.png'を論理名'diagram'として展開し、出力テンプレートを'/workspace/figures/diagram.png'へ解決する", () => {
