...
import { planPdfPageItems } from '@graphics-workbench/core/conversion';
...
describe('ページ数と出力テンプレートからPDFページごとの出力パスを生成する処理', () => {
  ...
  it('page countが2と${page}を含む出力テンプレートから、ページ1とページ2の変換処理単位を生成してsource-1.pngとsource-2.pngへ割り当てる', () => {
    const items = planPdfPageItems(source, 2, '${fileDirname}/${fileBasenameNoExtension}-${page}.png', ['.png']);

    assert.deepStrictEqual(
      items.map(({ page, outputPath }) => ({ page, outputPath })),
      [
  ...
  it('複数ページを分割する出力テンプレートに${page}が含まれない場合は変換処理単位を展開せず、${page}必須のエラーで拒否する', () => {
    assert.throws(
      () => planPdfPageItems(source, 2, '${fileDirname}/${fileBasenameNoExtension}.png', ['.png']),
  ...
  it('選択pageが空、0、負数、小数、page count超過の場合は出力パスを作らず拒否する', () => {
    ...
    assert.throws(() => planPdfPageItems(source, 12, template, ['.png'], []), /At least one PDF page/u);
    for (const page of [0, -1, 1.5, 13]) {
      assert.throws(() => planPdfPageItems(source, 12, template, ['.png'], [page]), /outside the range 1-12/u);
    }
  });
  ...
  it('page countが負数、小数、NaNの場合はPDFにpageが無いエラーで拒否する', () => {
    ...
    for (const pageCount of [-1, 1.5, Number.NaN]) {
      assert.throws(
        () => planPdfPageItems(source, pageCount, template, ['.png']),
        new RegExp(`PDF has no pages: ${RegExp.escape(source.sourcePath)}`),
      );
    }
  });
