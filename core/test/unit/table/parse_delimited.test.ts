...
describe('CSVパーサー', () => {
  ...
  it('ダブルクォートで囲まれたカンマを含むフィールドを1つのフィールドとして扱う', () => {
    assert.deepEqual(parseCsv('a,"b,c",d'), [['a', 'b,c', 'd']]);
  });

  it('""でエスケープされた引用符をフィールドへ含める', () => {
    assert.deepEqual(parseCsv('"say ""hi""",x'), [['say "hi"', 'x']]);
  });

  it('CRLFとLFの両方を行区切りとして扱う', () => {
...
describe('TSVパーサー', () => {
  ...
  it('CRLFを行区切りとして扱い、末尾の改行を余分な行にしない', () => {
  ...
  it('空文字列と改行のみの入力は0行を返す', () => {
    assert.deepEqual(parseTsv(''), []);
    assert.deepEqual(parseTsv('\n'), []);
  });

  it('タブを含む場合のみtable candidateと判定する', () => {
