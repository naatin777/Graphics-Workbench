...
import { planRasterFrameItems } from '@graphics-workbench/core/conversion';
...
describe('ラスター画像から出力テンプレートに従った変換処理単位を生成する処理の出力パス検証', () => {
  it('出力templateの拡張子が.jpegで許容拡張子が.pngのみの場合、"Invalid output extension"エラーで変換前に拒否する', async () => {
    await using workspacePath = await mkdtempDisposable(path.join(os.tmpdir(), 'gw-frame-items-'));
  ...
  it('出力templateの拡張子が.pngで許容拡張子が.pngの場合、変換処理単位を1件生成し出力パスの拡張子が.pngになる', async () => {
    ...
    await copyFile(testDataPath, sourcePath);
    const items = await planRasterFrameItems({
      sourcePath,
      workspacePath: workspacePath.path,
      workspaceName: path.basename(workspacePath.path),
    ...
    assert.strictEqual(path.extname(items[0]?.outputPath ?? ''), '.png');
  });
});
