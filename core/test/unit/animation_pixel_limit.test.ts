...
suite('アニメーションpixel上限invariant（core所有）', () => {
  test('合計pixel（width×pageHeight×frameCount）が上限内なら通過し、超過時はBigIntで総計を検査して例外を投げる', () => {
  ...
  test('寸法が0または非整数の場合は上限判定へ進まず、safe dimensionsの例外を投げる', () => {
    assert.throws(
  ...
  test('上限超過エラーは入力パス・設定値・合計pixelを報告する', () => {
