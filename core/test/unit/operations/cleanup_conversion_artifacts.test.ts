...
describe('変換結果を一時保存した作業ディレクトリの削除と、commit失敗時に元出力を退避した復旧バックアップ（.previous）の保持', () => {
  it('cleanup対象root内のresult.pdfを削除し、attempted 1・succeeded 1・failures 0件を返してOutput Channelへは何も記録しない', async () => {
  ...
  it('cleanup対象rootがworkspace外へ向くsymlinkの場合は失敗として扱い、失敗root pathをfailuresへ返し、Output Channelへ「[cleanup] failed」と「1/1 artifact roots failed」を記録する', async () => {
    await using workspacePath = await mkdtempDisposable(path.join(os.tmpdir(), 'gw-cleanup-workspace-'));
  ...
  it('上書きcommit直後のキャンセルとrollback copy失敗が重なった場合はCommitRollbackErrorを返し、commit済み出力を残して元出力のrecovery backup（result.pdf.previous）を保持する', async () => {
    await using workspacePath = await mkdtempDisposable(path.join(os.tmpdir(), 'gw-cleanup-workspace-'));
    const rootPath = path.join(workspacePath.path, '.graphics-workbench', 'run');
  ...
  it('通常のエラーで失敗した場合はrecovery backup名（.previous）を含めて一時作業ディレクトリ全体を削除する', async () => {
    await using workspacePath = await mkdtempDisposable(path.join(os.tmpdir(), 'gw-cleanup-workspace-'));
    const rootPath = path.join(workspacePath.path, '.graphics-workbench', 'run');
  ...
  it('rollbackが成功した場合はrecovery backupを残さずに一時作業ディレクトリごと削除し、元出力だけを復元する', async () => {
    await using workspacePath = await mkdtempDisposable(path.join(os.tmpdir(), 'gw-cleanup-workspace-'));
    const rootPath = path.join(workspacePath.path, '.graphics-workbench', 'run');
    const stagedOutputPath = path.join(rootPath, 'result.pdf');
    const outputPath = path.join(workspacePath.path, 'result.pdf');
  ...
  it('cleanup時はpreservePathsで指定したUndo用backup（result.pdf.previous）だけを残し、一時作業ディレクトリ内の変換結果result.pdfと入力コピーのsource.pdfを削除する', async () => {
    await using workspacePath = await mkdtempDisposable(path.join(os.tmpdir(), 'gw-cleanup-workspace-'));
    const rootPath = path.join(workspacePath.path, '.graphics-workbench', 'run');
  ...
  it('cleanup対象rootがworkspace外へ解決するsymlinkの場合は、symlink自体も参照先のファイルも削除しない', async () => {
    await using workspacePath = await mkdtempDisposable(path.join(os.tmpdir(), 'gw-cleanup-workspace-'));
    await using outsidePath = await mkdtempDisposable(path.join(os.tmpdir(), 'gw-cleanup-outside-'));
  ...
  it('cleanupが失敗しても呼び出し側へ例外を伝播させず、workspace内の変換出力output.pdfを削除しない', async () => {
    await using workspacePath = await mkdtempDisposable(path.join(os.tmpdir(), 'gw-cleanup-workspace-'));
    await using outsidePath = await mkdtempDisposable(path.join(os.tmpdir(), 'gw-cleanup-outside-'));
  ...
  it('cleanupは指定root配下のresult.pdfだけを削除し、別の変換セッションの進行中作業結果（other-active/result.pdf）・対象外の未知ディレクトリ・テスト用停止ログ（.graphics-workbench/harness/stop.log）・workspace外symlinkとその参照先ファイルをそのまま残す', async () => {
    await using workspacePath = await mkdtempDisposable(path.join(os.tmpdir(), 'gw-cleanup-workspace-'));
