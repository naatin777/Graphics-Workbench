...
describe('変換処理ごとに採番するIDが一時ディレクトリのパス要素として安全かを検証する', () => {
  it('安全な文字列の実行IDはpath segmentとして受け入れ、検証済みIDとしてそのまま返す', () => {
    assert.equal(isSafePathSegment('test-run_1.2'), true);
    assert.equal(asRunId('test-run_1.2'), 'test-run_1.2');
  });

  it('実行IDを時刻+UUID形式で生成すると、数字+UUID形式の実行IDで安全なpath segmentとなる', () => {
  ...
  it('..などの相対path、/や\\を含むpath、Windows予約名CON.pdf、129文字以上のsegmentはpath segmentとして拒否する', () => {
  ...
  it('sanitizePdfPathSegmentは..や.やCONなどの危険なbasenameを"pdf"へ、日本語などの非ASCIIを含むbasenameをアンダースコアへ置き換える', () => {
