...
| Playwright      | PR、main push        | skipしない         | Linux / macOS / Windows | Linux: `xvfb-run npm run test:playwright:vsix -- --project=vscode-electron electron/packaged_conversion_smoke.spec.ts`; macOS / Windows: `npm run test:playwright:vsix -- --project=vscode-electron electron/packaged_conversion_smoke.spec.ts`（3 OSとも4 cases smoke: Crop / PNG→JPEG / PDF→JPEG / Draw.io→PDF） | Playwright report / test-results              | 3 OS wide packaged conversion smoke（Host bridge、native dependency、外部CLI成功）               |
| Release package | tag                  | 対象外             | Linux / macOS / Windows | 全OS: `npm run test:playwright:vsix`（38 cases E2E）+ `npm run visual:capture`（各OS review screenshot、wide/narrow別）                                                                                                                                                                                            | `test-results/`, visual review artifact, VSIX | runner-matched artifact, native dependency, successful packaged conversion, manual visual review |
...
| `npm run test:playwright:vsix`                                                                         | packaged `graphics-workbench.vsix`のwide / narrow full E2E suite                              | Browser、Host Mocha、visual capture | full installed VSIX E2E journey                            |
| `npm run visual:capture`                                                                               | packaged `graphics-workbench.vsix`のWebviewをテーマ別に撮影し`artifacts/visual-review/`へ出力 | E2Eの合否判定には影響しない         | manual visual review用reference生成                        |
| `npm run test:playwright:vsix -- --project=vscode-electron electron/packaged_conversion_smoke.spec.ts` | macOS / Windowsではwideの4 packaged conversion smoke                                          | Browser、Host Mocha、narrow UI      | cross-platform artifact / bridge / native conversion smoke |
...
| CI-GAP-004 | Electron E2EはreleaseとPRでcase数が異なる（PRは3 OS smoke 4 cases、releaseは3 OS full 38 cases） | releaseとPRのcase差を見落とす                       | workflow commandとrunner-matched VSIX artifactを同期し、OS別Evidenceを正本にする |
...
- pull request description: `## Verification` sectionはPR templateで案内し、CIで強制しない。PR本文のローカルパス漏洩は、templateのPrivacy checklistで防ぐ（PR作成後のCI検知は漏洩を防げないため実施しない）
