...
- 目視確認用画像は`npm run visual:capture`（`visual_review_capture.spec.ts`）で`page.screenshot()`として明示的に生成し、`artifacts/visual-review/<platform>-<arch>/<viewport>/<screen>-<state>.png`へ出力する（`<viewport>`は`wide`または`narrow`。Electronを起動せず、Webview workspaceのVite dev serverにPlaywright Chromiumで接続し、`?theme=`のURL切替でテーマを適用する。`wide`撮影後に`page.setViewportSize()`で`narrow`へリサイズして再び撮影する）
- generated画像（`artifacts/`）はGit管理しない。人間が確認して採用したreference画像は`vscode/webview/e2e/visual-review/references/`へGit管理する
- スクリーンショット生成は通常のE2Eコマンド（`test:e2e`）から分離し、E2Eの合否判定に使わない
...
- [`vscode/webview/e2e/visual_review_capture.spec.ts`](../../vscode/webview/e2e/visual_review_capture.spec.ts)
