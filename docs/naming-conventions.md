...
| command ID                   | extension namespace + capability + mode     | 既存形式の`lowerCamelCase`/dot              | `graphics-workbench.splitPdf.allPages`                            | `graphics-workbench.convert`（粒度が広すぎる）     | public IDは互換性のため既存名を維持する。manifest外のIDをruntime登録しない             |
