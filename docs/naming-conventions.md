...
| config key                   | 設定domain + semantic setting               | `lowerCamelCase`/dot                        | `outputPath.convertPngToPdf`、`outputPath.convertPdfToPng`        | `outputPath.convertToPdf`                          | command IDとoutput path keyは別の粒度で命名し、pair-specific keyを正本にする           |
| output template              | 出力の対象と粒度を示す                      | pair-specific output path                   | `outputPath.convertPngToPdf`、`outputPath.convertPdfToPng`        | `outputPaths.convertPdfToPng`                      | 単一出力・複数出力とも個別の`outputPath.convertXToY`を使う                             |
