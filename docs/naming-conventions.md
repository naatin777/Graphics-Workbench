...
| command ID                   | extension namespace + capability + mode     | 既存形式の`lowerCamelCase`/dot              | `graphics-workbench.splitPdf.allPages`                                | `graphics-workbench.convertPngToPdf`をgeneric PDFのcanonical名にする | public IDは互換性のため既存名を維持し、canonical internal symbolと分ける     |
