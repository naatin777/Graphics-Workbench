...
`graphics-workbench.convertToPdf`は追加済みだが、現状の`package.json`には旧入力形式別PDF変換コマンドがまだ公開UIとして残っている。
...
- `package.json`の`contributes.commands`に`graphics-workbench.convertToPdf`が残ることをテストする
...
- Explorer context menuに`変換`サブメニューが存在し、その配下に`graphics-workbench.convertToPdf`が表示されることをテストする
