...
- 長幅と短幅の2つのElectron projectで同じ17ケースを実行する
...
- [x] 長幅・短幅のPlaywright projectが同じ17ケースを列挙する
...
- Electronの終了処理をプロセスツリー全体へ拡張し、製品仕様外の終了ダイアログを期待値にせず、直接終了させる経路にした。E2E起動時はCrashpadも無効化し、対象VS Codeプロセスの孤児化を増やさない。
- `playwright.config.mjs` に長幅 `1280px` と短幅 `600px` の2 projectを追加し、`npx playwright test --list --project=vscode-electron --project=vscode-electron-narrow` で3ファイル・34ケース（17 × 2幅）を確認した。Cropのボタン式ズーム、Splitの数値入力・Ctrl/Command+ホイールズームを実Electronで確認する回帰テストも追加した。
...
- `npx playwright test --update-snapshots=all --reporter=line`: 30 passed (2.7m)。wide/narrowのPNGを更新し、Crop/SplitのPDF中央寄せとMerge操作ボタンを目視確認した。続けて `npx playwright test --reporter=line` を実行し、30 passed (2.9m) を再確認した。ズーム回帰テスト追加後の34ケースは今回の検証結果へ追記する。
...
- ズーム回帰テストのwide/narrow実行は `4 passed (34.5s)`。Crashpad無効化後の同実行では、リポジトリのVS Codeプロセス数が実行前後で28件のまま増加しなかった。
- 現在のローカルDarwin全件再実行では、高コントラストCrop snapshotの1件が17,501 pixels（6%）差で失敗した。目視ではレイアウト崩れではなく文字アンチエイリアス・境界の差であり、Linux/Windowsを含むOS別snapshotはPRの再生成フローで更新する。
- 固定 `page.waitForTimeout` を禁止する `project/no-fixed-e2e-wait` と、安定化手順を `~/.codex/skills/graphics-workbench-e2e-stability/SKILL.md` に追加した。skill validatorも成功した。
