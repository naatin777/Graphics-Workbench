...
旧 `graphics-workbench.mergePdf` command IDは復元しない。v1.0.0へ向けた破壊的変更として、現行の `mergePdf.selectedFiles` へ移行する。
...
- 旧 `graphics-workbench.mergePdf` command IDは追加しない
...
- 旧 `graphics-workbench.mergePdf` command IDは追加していない。
