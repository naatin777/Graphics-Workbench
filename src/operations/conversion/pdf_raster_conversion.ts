...
import { parseSplitPdfPages, type SplitPdfPageParseResult } from '../../shared/protocols/split_pdf_protocol.js';
...
export function resolvePdfRasterPages(selection: PdfRasterPageSelection, pageCount: number): SplitPdfPageParseResult {
  ...
  const result = parseSplitPdfPages(selection.value, pageCount);
...
function formatPdfPageSelectionError(
  failure: Extract<SplitPdfPageParseResult, { ok: false }>,
  pageCount: number,
): string {
  switch (failure.kind) {
    case 'required': {
...
function pageSelectionFailure(
  kind: Extract<SplitPdfPageParseResult, { ok: false }>['kind'],
  token: string,
): Extract<SplitPdfPageParseResult, { ok: false }> {
