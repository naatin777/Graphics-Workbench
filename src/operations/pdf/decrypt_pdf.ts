...
async function decryptPdf(params: {
  ...
  const workDirectory = path.join(job.workspacePath, '.graphics-workbench', 'decrypt-pdf', runId, itemName);
  ...
  await runExternalTool({
    toolName: 'qpdf',
    executable: qpdfPath,
    args: ['--decrypt', `--password=${password}`, copiedSourcePath, stagedOutputPath],
    ...(signal === undefined ? {} : { signal }),
    redactArgument: (argument) => (argument.startsWith('--password=') ? '--password=<redacted>' : argument),
  });
  ...
  return {
    ...
    stagingRootPath: path.join(job.workspacePath, '.graphics-workbench', 'decrypt-pdf', runId),
  };
}
