...
export const strictSourceRules = {
...
/*
 * Rules provided by the local oxlint JS plugin (scripts/oxlint-project-plugin.mjs).
 * The plugin only runs under package configs that declare it, so these rules are
 * exported for package configs to spread in rather than kept in the root config
 * (which lints scripts without the plugin).
 */
...
export default defineConfig({
  ...
  rules: {
    ...
    /*
     * Project-specific rules are provided by the local oxlint JS plugin and
     * are spread into package configs that declare the plugin (see
     * `projectRules` above). The root config lints scripts only, so it must not
  ...
  overrides: [
    ...
    },
    {
      ...
      files: ['scripts/**/*.mjs', '.github/scripts/**/*.mjs', 'playwright.config.mjs'],
      rules: {
        'typescript/no-unsafe-assignment': 'off',
        'typescript/no-unsafe-call': 'off',
        'typescript/no-unsafe-member-access': 'off',
    ...
    },
    {
      ...
      files: ['core/scripts/generate-test-output.ts', 'scripts/generate-test-output.ts'],
      rules: {
        'eslint/no-unreachable-loop': 'off',
      },
    },
