...
export function App(): JSX.Element {
  ...
  function handleInsert(): void {
    vscode.sendMessage({ type: 'insert', payload: { format: format(), code: preview() } });
  }

  onMount(() => {
    const onMessage = (event: MessageEvent<TableEditorHostToWebview>): void => {
      const message = event.data;
      if (!isTableEditorHostToWebviewMessage(message)) {
        return;
      }
      ...
      setFormat(message.payload.format);
