...
suite('TIFF変換planner', () => {
  test('2ページPDFをページごとのTIFF jobへ展開する', async () => {
    ...
    try {
      ...
      const jobs = await planTiffConversionJobs(vscode.Uri.file(sourcePath), getCommandConfiguration(), 100_000_000);
  ...
  test('通常のTIFF入力を同一形式変換として拒否する', async () => {
    ...
    await assert.rejects(
      planTiffConversionJobs(vscode.Uri.file(sourcePath), getCommandConfiguration(), 100_000_000),
