...
const fixturePath = path.resolve('test/input/valid/pdf/single-page-document.pdf');
...
suite('Terminal frontend用PDF→raster use case', () => {
  ...
  test('選択pageと既存output templateからpage付きRasterInputだけを計画する', async () => {
    ...
    const outputTemplate = getDefaultConfiguration().outputPath.split.jpeg();
  ...
  test('既存raster operationでPDFをPNG変換し、Undoを持たないfrontendの成功後にstagingを掃除する', async () => {
    ...
    const plan = planPdfRasterConversion({
      ...
      target: 'png',
      selection: { kind: 'all' },
      outputTemplate: getDefaultConfiguration().outputPath.split.png(),
    });
