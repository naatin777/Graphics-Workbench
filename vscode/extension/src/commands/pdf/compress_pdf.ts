...
export async function compressPdfCommand(sourceUris: vscode.Uri[], dependencies: CommandDependencies): Promise<void> {
  ...
  if (sourceUris.length === 0) {
    await vscode.window.showErrorMessage(userMessage('message.compressPdf.failed', 'No PDF files were selected.'));
    return;
  }
  ...
  await runConversionLifecycle({
    ...
    messages: {
      progressTitle: userMessage('message.progress.compressPdf.title', sourceUris.length),
    ...
    run: async (runtime) => {
      ...
      return toConversionResult(async () => compressPdfFiles({ inputs, runtime }), runtime.signal);
    },
  });
}
