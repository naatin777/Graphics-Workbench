...
async function assertTestWorkspaceIsEmpty(): Promise<void> {
  ...
  assert.deepStrictEqual(entries, [], 'vscode/test/support/workspace must be empty at the test boundary');
}
...
function resolveWorkspacePath(relativePath: string): string {
  ...
  if (
    ...
    throw new Error(`Workspace destination must be inside vscode/test/support/workspace: ${relativePath}`);
  }
