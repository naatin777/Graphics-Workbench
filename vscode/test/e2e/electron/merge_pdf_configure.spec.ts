...
test('公開UIでMerge順を変更してConfigureをキャンセルできる', async ({ playwright }, testInfo) => {
  testInfo.setTimeout(120_000);
  let env: ElectronTestEnv | undefined;
  const consoleMessages: string[] = [];
  ...
  try {
    ...
    const { frame } = await openMergePdfConfigure(env.app.window, [cropConfigureFixture.fileName, secondPdfName]);
    const sourceNames = frame.locator('.source-card h3');
    await expect(sourceNames).toHaveText([cropConfigureFixture.fileName, secondPdfName]);

    await frame.getByRole('button', { name: 'Move down', exact: true }).first().click();
    await expect(sourceNames).toHaveText([secondPdfName, cropConfigureFixture.fileName]);

    await frame.getByRole('button', { name: 'Cancel', exact: true }).click();
    await expect.poll(() => env!.app.window.frames().includes(frame)).toBe(false);
  } catch (error) {
    await attachElectronDiagnostics({
      consoleMessages,
      error,
